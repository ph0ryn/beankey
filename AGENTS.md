...
- `docs/spec.md` は、対象範囲、外部から観測できる要件、現在の正常動作検証、将来の原作適合検証および完了条件の正本とする。
- `docs/architecture.md` は、コンポーネント、依存方向、プロセス境界およびNixOSへの統合方法の正本とする。
- `docs/research.md` は、固定した原作から確認した事実、推論および未確認事項を記録する。
- このファイルは、実装時に守る責務とリポジトリ境界を定義する。確定した設計を変更する場合は、対応する文書とこのファイルを同時に更新する。
